...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop org.firedroid.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add gitlab https://gitlab.com/firedroid/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.firedroid.device | grep -q "$CUSTOM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.firedroid.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
